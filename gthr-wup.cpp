...
static_assert(sizeof(OSThread *) <= sizeof(__gthread_t),
              "__gthread_t overflow");
static_assert(sizeof(OSCondition) <= sizeof(__gthread_cond_t),
              "__gthread_cond_t overflow");
static_assert(sizeof(void *) <= sizeof(__gthread_key_t),
              "__gthread_cond_t overflow");
static_assert(sizeof(OSMutex) <= sizeof(__gthread_mutex_t),
              "__gthread_cond_t overflow");
static_assert(sizeof(u32) <= sizeof(__gthread_once_t),
              "__gthread_once_t overflow");
static_assert(sizeof(OSMutex) <= sizeof(__gthread_recursive_mutex_t),
              "__gthread_cond_t overflow");
...
int __gthread_recursive_mutex_init_function(
...
int __gthread_create(__gthread_t *_thread, void *(*func)(void *), void *args) {
  ...
  OSThread *thread = (OSThread *)MEMAllocFromExpHeapEx(
      (MEMExpandedHeap *)MEMGetBaseHeapHandle(MEM_BASE_HEAP_MEM2),
  ...
  if (!OSCreateThread(thread, OSThreadEntryPointFnWrap, 2, (char **)argv,
                      stack + __GTHREADS_STACK_SIZE, __GTHREADS_STACK_SIZE,
                      OS_THREAD_DEF_PRIO, attr)) {
    MEMFreeToExpHeap(
        (MEMExpandedHeap *)MEMGetBaseHeapHandle(MEM_BASE_HEAP_MEM2), thread);
    return EINVAL;
  }
...
int __gthread_join(__gthread_t thread, void **value_ptr) {
  if (!OSJoinThread((OSThread *)thread, (int *)value_ptr))
    return EINVAL;
  MEMFreeToExpHeap((MEMExpandedHeap *)MEMGetBaseHeapHandle(MEM_BASE_HEAP_MEM2),
                   thread);
  return 0;
}
...
__gthread_t __gthread_self(void) { return OSGetCurrentThread(); }
...
int __gthread_cond_timedwait(__gthread_cond_t *cond, __gthread_mutex_t *mutex,
                             const __gthread_time_t *abs_timeout) {
  OSWaitCond((OSCondition *)cond, (OSMutex *)mutex);
  return 0;
}

int __gthread_active_p(void) { return 1; }
