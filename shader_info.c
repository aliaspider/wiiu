...
int GX2PixelShaderInfo(GX2PixelShader *ps, char *buffer) {
   ...
   ptr += sprintf(ptr, "output0_enable: 0x%X\n", ps->regs.cb_shader_mask.output0_enable);
   ptr += sprintf(ptr, "rt0_enable: %s\n", ps->regs.cb_shader_control.rt0_enable ? "TRUE" : "FALSE");
   ptr += sprintf(ptr, "z_order: %s\n",
                  (ps->regs.db_shader_control.z_order == db_z_order_late_z)
                     ? "db_z_order_late_z"
                     : (ps->regs.db_shader_control.z_order == db_z_order_early_z_then_late_z)
                          ? "db_z_order_early_z_then_late_z"
                          : (ps->regs.db_shader_control.z_order == db_z_order_re_z)
                               ? "db_z_order_re_z"
                               : (ps->regs.db_shader_control.z_order == db_z_order_early_z_then_re_z)
